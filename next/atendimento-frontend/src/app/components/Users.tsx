import { Button, HStack, DataList, Box, Center, ButtonGroup, Input, Checkbox, Alert, CloseButton } from "@chakra-ui/react"
...
export function Users() {
  return (
      <Box>
        <Center p="4">Users ({stats.length})</Center>
        <Center>
        ...
        <DataList.Root orientation="horizontal" divideY="1px" width="100%" height="100%">
          {stats.map((item) => (
            <DataList.Item key={item.id}>
              <DataList.ItemValue>{item.firstName} {item.lastName}</DataList.ItemValue>
              <Checkbox.Root>
                <Checkbox.HiddenInput />
                <Checkbox.Label>Block user?</Checkbox.Label>
                <Checkbox.Control />
              </Checkbox.Root>
